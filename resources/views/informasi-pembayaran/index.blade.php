<x-app-layout>
                ...
                <div class="bg-gray-50 p-4 rounded-md">
                    <form class="space-y-6 flex flex-col">
                        <div class="flex items-center">
                            <label for="periode_ppdb" class="w-32  block text-sm font-medium text-gray-700">Periode PPDB</label>
                            <select onchange="handlePeriodeChange()" value="{{$periodePPDB[0]->id_periode}}" name="periode_ppdb" id="periode_ppdb" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div class="flex items-center">
                            <label for="detail_pembayaran" class="w-32  block text-sm font-medium text-gray-700">Periode PPDB</label>
    ...
    <script>
        let informasiPembayaran = [];
        function handlePeriodeChange() {
            let periodeId = document.getElementById('periode_ppdb').value;
            let url = "{{ route('informasi-pembayaran.show', ':periodeId') }}";
